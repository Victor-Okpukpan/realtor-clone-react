    ...
    <div className="bg-white border-b shadow-sm sticky top-0 z-40">
        <header className="flex justify-between px-3 max-w-6xl mx-auto items-center">
            ...
            <div className="sm:hidden flex flex-1 justify-end items-center py-3">
                ...
                <div className={`${!toggle ? "hidden" : "flex"} p-6 bg-gradient-to-r from-gray-700 via-gray-900 to-black absolute top-12  right-0 mx-4 my-2 min-w-[140px] z-10 rounded-xl transition duration-200 ease-in-out`}>
                <ul className="flex justify-end items-start flex-col ">
                    <li className={`cursor-pointer py-3 text-sm font-semibold  border-b-[3px]  ${pathMatchRoute("/") ? "text-black border-none" : "text-gray-400 border-b-transparent"}`} onClick={()=>navigate("/")}>Home</li>
                    <li className={`cursor-pointer py-3 text-sm font-semibold border-b-[3px] ${pathMatchRoute("/offers") ? "text-black border-none" : "text-gray-400 border-b-transparent"}`} onClick={()=>navigate("/offers")}>Offers</li>
                    <li className={`cursor-pointer py-3 text-sm font-semibold border-b-[3px] ${(pathMatchRoute("/sign-in") || pathMatchRoute("/profile")) ? "text-black border-none" : "text-gray-400 border-b-transparent"}`} onClick={()=>navigate("/profile")}>{pageState}</li>
                </ul>
                </div>
            </div>
        </header>
